...
test("it shows the title in the banner", () => {
  setup();
  // We expect that the title 'Hi, Iam Maisy' is in the banner component
  expect(screen.getByText(/Hi, Iam Maisy/i)).toBeInTheDocument();
});
...
test("can download cv when clicked on download cv button", async () => {
  ...
  // expect(downloadLink.download).toEqual('Maisy-Resume.pdf');
  // expect(downloadLink.href).toEqual('/files/Maisy-Resume.pdf');
